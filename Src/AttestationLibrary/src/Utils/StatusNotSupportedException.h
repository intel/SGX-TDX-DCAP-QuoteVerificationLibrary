/***************************************************************************
*                           INTEL CONFIDENTIAL                             *
*                     Copyright 2023 Intel Corporation.                    *
*                                                                          *
*  This  software  and  the  related  documents   are  Intel  copyrighted  *
*  materials,  and your  use of them  is governed by the express  license  *
*  under which they were provided  to you  (License).  Unless the License  *
*  provides otherwise, you may not use, modify, copy, publish, distribute, *
*  disclose or transmit  this software  or the related  documents without  *
*  Intel's prior written permission.                                       *
*                                                                          *
*  This software and  the related documents are  provided as is,  with no  *
*  express  or implied  warranties,  other than those that  are expressly  *
*  stated in the License.                                                  *
****************************************************************************/

#ifndef SGXECDSAATTESTATION_STATUSNOTSUPPORTEDEXCEPTION_H
#define SGXECDSAATTESTATION_STATUSNOTSUPPORTEDEXCEPTION_H

#include <exception>

namespace intel { namespace sgx { namespace dcap {
/**
 * Exception when Enclave Report having ISVSVN on level 1 and with enclave identity having tcbs on 2,3,4
 */
class StatusNotSupportedException : public std::exception
{
public:
    explicit StatusNotSupportedException(const std::string message_) : message(message_) {}
    const char *what() const noexcept override{ return this->message.c_str(); }
protected:
    std::string message;
};

}}} // namespace intel { namespace sgx { namespace dcap {
#endif //SGXECDSAATTESTATION_STATUSNOTSUPPORTEDEXCEPTION_H
